...
inline __device__ List::List(unsigned size) {
...
inline __device__ void List::init(unsigned *mem, unsigned size, unsigned cap) {
	array = mem;
	nitems = size;
	capacity = cap;
}
inline __device__ void List::push(unsigned item) {
...
inline __device__ unsigned *List::toArray() {
	return array;
}
inline __device__ void List::clear() {
	if (array) free(array);
	nitems = 0;
	capacity = 0;
}
inline __device__ unsigned List::size() {
	return nitems;
}
inline __device__ void List::uniq(unsigned *mark, unsigned maxelement) {
	unsigned id = blockIdx.x * blockDim.x + threadIdx.x;
